...
class NPUProcessor(threading.Thread):
    ...
    def _enqueue_crops(self, frame, dets):
        ...
        Non-blocking: if queue is full the oldest slot is dropped (detection
        must never wait on recognition)."""
        h, w = frame.shape[:2]
        for d in dets:
            ...
            try:
                ...
                # Drop the oldest entry and insert the new one
                try:
                ...
                try:
                    ...
                    pass  # Give up gracefully — never block detection
